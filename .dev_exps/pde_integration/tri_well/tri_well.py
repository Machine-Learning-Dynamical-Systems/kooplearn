...
def generate_dataset(cfg: BaseConfigs, rng_seed: int = 42):
    ...
    traj = ds.generate(np.array([-0.5]), cfg.num_samples, show_progress=True)
    return traj[::cfg.take_every], ds._ref_eigenvalues, ds._ref_boltzmann_density, ds._ref_domain_sample
...
def main(cfg: BaseConfigs):
    training_traj, ref_eigenvalues, ref_density, domain_sample = generate_dataset(cfg)
    ...
    model = model.fit(training_traj[:-1], training_traj[1:])
    eigenvalues = np.sort(model.eig())
    print(eigenvalues)
