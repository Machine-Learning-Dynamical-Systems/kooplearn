...
class Kernel(BaseEstimator):
    ...
    *infinite-dimensional nonlinear* feature map associated to a kernel 
    ...
    _parameter_constraints: dict = {
        ...
        "eigen_solver": [
            StrOptions({"auto", "dense", "arpack", "randomized"})
        ],
        ...
        if self.reduced_rank:
            ...
            if eigen_solver == "randomized":
                ...
                fit_result = regressors.pcr(
                    self.kernel_X_, self.alpha, n_components, eigen_solver
                )
    ...
    def predict(self, X, n_steps=1, observable=None):
        """Predicts the state or, if the system is stochastic, its expected
        ...
        state_pred = regressors.predict(
            n_steps, self.U_, self.V_, self.kernel_YX_, K_Xin_X, X_fit
        )

        if observable is not None:
            observable = validate_data(
                self, observable, reset=False, copy=self.copy_X
            )
    ...
    def risk(self, X=None):
        ...
        return regressors.estimator_risk(
    ...
    def modes(self, X, observable=None):
        ...
        _gamma = regressors.estimator_modes(eig_result, K_Xin_X)
