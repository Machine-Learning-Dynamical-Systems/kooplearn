...
class DPNet(TrainableFeatureMap):
    ...
    def __call__(self, X: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            embedded_X = self.lightning_module.encoder(X.to(self.lightning_module.device))
...
class DPModule(lightning.LightningModule):
    def __init__(
        ...
        self.save_hyperparameters(ignore=["encoder", "optimizer_fn", "kooplearn_feature_map_weakref"])
