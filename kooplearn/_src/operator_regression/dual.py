...
def fit_rand_tikhonov(
    ...
    alpha = dim*tikhonov_reg
    tikhonov = np.identity(dim, dtype=K_X.dtype) * alpha
    V, S, _ = randomized_svd(K_X + tikhonov, rank, n_oversamples=n_oversamples, n_iter=iterated_power, random_state=rng_seed)
    S, V = _postprocess_tikhonov_fit(S, V, rank, dim, rcond)
    if _return_singular_values:
        return V, V, S
    else:
...
def _postprocess_tikhonov_fit(
    ...
    if all(_test):            
        ...
        logging.warn(f"The numerical rank of the projector ({V.shape[1]}) is smaller than the selected rank ({rank}). {rank - V.shape[1]} degrees of freedom will be ignored.")
...
def evaluate_eigenfunction(
        ...
        U_or_V: ArrayLike, #Projection matrix: first output of the fit functions defined above (right eigenfunctions) or second output (left eigenfunctions)
