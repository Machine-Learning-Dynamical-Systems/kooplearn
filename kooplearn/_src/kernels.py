...
class ScalarProduct(BaseKernel):
    # A base class for scalar product kernels, to be subclassed specifying the feature map. !!
    # Covariance matrices are averaged, but kernels are not!!
    def __call__(self, X, Y=None):
        """Return the Kernel matrix k(x,y) := <Phi(x),Phi(y)>. Different samples are batched on the first dimension
        of X and Y."""
...
class ExpSineSquared(BaseKernel):
    def __init__(self, length_scale=1.0, periodicity=1.0):
        ...
        self._scikit_kernel = sk_kernels.ExpSineSquared(length_scale=self.length_scale, periodicity=self.periodicity,
