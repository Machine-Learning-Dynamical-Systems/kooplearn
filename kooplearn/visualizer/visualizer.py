...
class Visualizer:
    ...
    def compute_mode_info(self, deltat:float, xcoord=None, ycoord=None):
        ...
        self.infos['eig_num'] = np.repeat(np.arange(n_eigs), n_features)    # Identifying the mode number
        self.infos['var_index'] = np.tile(np.arange(n_features), n_eigs)    # Unique identifier for each variable
        if xcoord is None:                                                  # If no specific coordinate is given, each dimensionis
            self.infos['x'] = np.tile(np.arange(n_features), n_eigs)        #   identified with its index in the observable array
        else:                                                               # Otherwise coordinates are saved for accurate plotting
            self.infos['x'] = xcoord
        if not ycoord is None:                                              # If y coordinates make sense, save them as well
            self.infos['y'] = ycoord
        ...
        self.infos['modulus'] = np.repeat(np.abs(eigs)**(1/deltat), n_features) # Modulus of the mode
        angles = np.log(eigs)
        ...
        self.infos['phase'] = np.arctan(Z.imag/Z.real)                      # Phase of the mode at every point
