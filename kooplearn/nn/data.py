...
class ContextsDataset(Dataset):
    def __init__(self, contexts: torch.Tensor):
        if not isinstance(contexts, torch.Tensor):
            logger.warning(f'The provided contexts are of type {type(contexts)}. Converting to torch.Tensor.')
            contexts = torch.as_tensor(contexts)
        if contexts.ndim < 2:
            raise ShapeError(f'Invalid contexts shape {contexts.shape}. The contexts must be at least three dimensional [num_samples, context_len, *features].')
...
class TrajToContextsDataset(ContextsDataset):
    def __init__(self, trajectory: torch.Tensor, context_window_len: int = 2, time_lag: int = 1):
        ...
        if not isinstance(trajectory, torch.Tensor):
            logger.warning(f'The provided trajectory is of type {type(trajectory)}. Converting to torch.Tensor.')
