...
def error_decay_variable(n: int, delta: float, kernel_sup_norm: float = 1.0) -> float:
    return (n**-1)*kernel_sup_norm*np.log(np.pow(n, 2)*8*(delta**-1))

def hat_eps_n(X: np.ndarray, kernel: Kernel, delta: float, kernel_sup_norm: float = 1.0) -> float:
    J = error_decay_variable(X.shape[0], delta, kernel_sup_norm=kernel_sup_norm)
    cov_norm = np.linalg.norm(kernel(X), ord=2) #Operator norm of the Gram Matrix
